...
class ServiceMock:
    container_name = ''
    hostname = ''
    hostcode_dir = os.getenv('HOST_CODE_DIR')

    def start_container(self):
        command = ['docker', 'run', '-d',
                   '--name', self.container_name,
                   '-h', self.container_name,
                   '-v', self.hostcode_dir+":{1}".format(os.getcwd(), "/data/"),
                   '-e', 'FLASK_APP=/data/test/json-server/server.py',
                   '10.56.2.48/devops/weaveworksdemos/json-server',
                   '--port', '80']
...
class OrdersContainerTest(unittest.TestCase):
    ...
    def test_api_validated(self):
        print('Startint test api validate')
        limit = 30
...
if __name__ == '__main__':
    ...
    OrdersContainerTest.COMMIT = os.getenv("COMMIT")
